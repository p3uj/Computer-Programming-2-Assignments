...
int main(){
    ...
    for (i=0; i<5; i++){                            // Input and Processing
        ...
        for (j=0; j<3; j++){                        // For quizzes
            ...
            gradeAve[i] += stud_rec[i].quiz[j];     // Accumulate all the values of quizzes (row-wise)
        }                                           //
        gradeAve[i] = gradeAve[i]/3;                // Computing the grade average
    }                                               // End
    // Computing the sum of column and average start here
    for (i=0; i<4; i++){    // Column
        for (j=0; j<5; j++){    // Row
            if (i<3)
                average[i] += stud_rec[j].quiz[i];   // Accumulate all the values of quizzes (column-wise)
            else
                average[i] += gradeAve[j];           // Accumulate all the values of grade average
        }
        if (i<3)
            average[i] = average[i]/5;               // Computing the average of each quizzes
        else
